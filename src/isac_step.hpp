...
namespace sac {
  ...
  protected:
    ...
    iter_1d it1_1d, low, up;   size_t j, steps, length, length2;
    state_type switch_times_within_range;
  
  public:
    /* public initialization */
    double J0, Jn, Jprev, t_i, t_f, t_app, tf, t_imp;    
    state_type x0noU, xnext, x_imp;  
    int its, i_, counter_;
    ...
    isac_step( ) : 
		 ...
		 x0noU(xlen), xnext(xlen) { impacts::set_PI_map(); }
    ...
    inline void simrho_u_default_with_resets();
	...
	if (( Jn > J0 )||isequal(t_i, t_f)) { // cost increased so don't apply control or switching times too close
		//std::cout <<"NO ACTION FOUND!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << "\n";
		xnext = x0noU;    // return x
		for (i_ = 0; i_ < ulen; ++i_) { u_switch[i_] = 0; } // return zero action
		t_i = t_f;//zero duration
		u.clear();//clears switching times and value in control instance
		Jn=J0;
	}
	else { 
		x_intp( t0+ts, xnext );
		t_imp = impacts::tm; //for plotting purposes
		x_imp = impacts::ximp;		
		} 		
  ...
  inline void isac_step::SimInitXRho( double &t0, const state_type &x0) {
	...
	//clear registered impacts from previous iteration
	impacts::imp_times.clear(); impacts::xim_m.clear(); impacts::xim_p.clear();
    simx_u_default( x, t0, tf);//simulate state with default control
    x_intp( t0+ts, x0noU );//interpolate in case no action is found
	t_imp = impacts::tm; x_imp = impacts::ximp;//for plotting purposes
	impacts::tm = t_final; //reset value for rest of iteration
    simrho_u_default_with_resets();//simulate costate with default control 
  }
  ...
  inline void isac_step::SimNewX( double &t0, const state_type &x0 ) {
	...
	simX_with_impacts( xdot, x, t_i, t_f, x_vec, times ); // u_new 
    //
    //After new action
    if (t_f-(tf)< -epsilon) {
      x_vec.pop_back(); times.pop_back();
  ...
  */
  inline void isac_step::simx_u_default( state_type& x, const double &tinit, const double &tfinal) {
	...
	for (j = 0; j < length-1; ++j) { // simulate with default control (initial condition x is automatically changed from the integrator)
		...
		simX_with_impacts( xdot, x, switch_times_within_range[j], switch_times_within_range[j+1], x_vec, times );
		if(j < length-2) { x_vec.pop_back(); times.pop_back(); }
	}
  }


  /*Piecewise simulate rho based on u_default and "switch_times_within_range" from simx_u_default and registered impacts*/
  inline void isac_step::simrho_u_default_with_resets() {	
	rho_vec.clear(); rho_times.clear(); 	
	//First add impact times in the switch times vector
	length2 = impacts::imp_times.size(); counter_ = length2-1;//used to count costate resets
	if(length2 > 0) {
		length = length + length2; //std::cout << "length: "<<length2 <<"\n";
		for ( j = 0; j < length2; ++j ) { switch_times_within_range.push_back(impacts::imp_times[j]); }
		std::sort (switch_times_within_range.begin(), switch_times_within_range.end());//sort them
	}
	//Final state
	m_mrho_tf = J1.get_dmdx( );
	for ( j = 0; j < xlen; ++j ) { rho[j] = m_mrho_tf[j]; }
	//Piecewise Integrate
	for (i_ = length-2; i_ > -1; i_--) { // simulate with default control (initial condition rho is automatically changed from the integrator or reset map)
		simRho( rho_dot, rho, switch_times_within_range[i_], switch_times_within_range[i_+1], rho_vec, rho_times ); 	
		//Apply reset
		if(counter_ >= 0) {
			if(isequal (switch_times_within_range[i_], impacts::imp_times[counter_])){
				impacts::PI_reset (switch_times_within_range[i_], counter_, rho, u);
				--counter_; 
				continue;//don't want to "pop" in this scenario
			}
		}
